        ...
        <script>
            document.getElementById("btn-mercado").addEventListener("click", async () => {
                ...
                const res = await fetch("http://localhost:3000/create_preference", {
                    ...
                    body: JSON.stringify({total}) // 👈 AQUÍ SE ENVÍA EL TOTAL
                });

                const data = await res.json();
                console.log("PREF ID:", data);
...
async function finalizarPedido() {
    ...
    if (!ubicacion) {
        alert("Debes validar tu ubicación antes de continuar.");
        return;
    }

    const form = new FormData();
    form.append("carrito", JSON.stringify(carrito));
    form.append("ubicacion", JSON.stringify(ubicacion));
    form.append("total", total);

    const res = await fetch("/TaqueriaLaCruz/controlador/crearPedido.php", {
        method: "POST",
        body: form
    });
