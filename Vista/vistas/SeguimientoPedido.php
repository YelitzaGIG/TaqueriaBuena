...
<main class="tracking-container">
  ...
  <p class="detalle">Entrega en: <strong id="direccion"><?= htmlspecialchars($data['direccion'] ?? 'Sin direcciÃ³n') ?></strong></p>
...
<script>
  ...
  // Asegurar que sean nÃºmeros vÃ¡lidos
  let clientePos = { 
    lat: Number(<?= json_encode($lat_pedido) ?>) || TAQUERIA.lat, 
    lng: Number(<?= json_encode($lng_pedido) ?>) || TAQUERIA.lng 
  };
  let repartidorPos = { lat: TAQUERIA.lat, lng: TAQUERIA.lng }; // inicia en taquerÃ­a
</script>
