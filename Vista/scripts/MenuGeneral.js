...
function verificarSesionYUbicacion() {
    ...
    console.log('Verificando sesión y ubicación:', { usuario, ubicacion });
    ...
    if (!ubicacion || !ubicacion.dentro_rango) {
        // Mostrar el modal de ubicación si no está validada
...
document.querySelectorAll('.plus-btn').forEach(btn => {
    btn.addEventListener('click', () => {
        ...
        if (!validacion.valido) {
            console.warn('Validación fallida:', validacion.mensaje);
            // No mostrar alert, el modal se mostrará automáticamente
            return;
        }
...
document.querySelector(".checkout").addEventListener("click", () => {
    ...
    const validacion = verificarSesionYUbicacion();
    if (!validacion.valido) {
        console.warn('No se puede proceder al pago:', validacion.mensaje);
        return;
    }
